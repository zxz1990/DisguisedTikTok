...
//    }
    ...
    fun getVideoUrls(context: Context): List<VideoData> {
        val list = context.assets.list("video")?.asSequence()?.filter {
            it.endsWith(".mp4")
        }?.map { path ->
            VideoData(
                uri = Uri.parse("file:///android_asset/video/$path"),
                coverUri = Uri.parse(
                    "file:///android_asset/video/${path.replace(
                        ".mp4",
                        "_thumb"
                    )}"
                )
